...
const getTotalRevenue = (orders) => {
  ...
  let totalRevenue = deliveredOrders?.reduce(
...
// 8. async fetchAndProcess() — wraps above in async/await,
// simulate 1s API delay with Promise, call all above functions and log results with console.table.

const fetchAndProcess = new Promise((resolve, reject) => {
  let AllFunctionsData = [
    { getOrderTotal: getOrderTotal(orders[0]) },
    { getDeliveredOrders: getDeliveredOrders(orders) },
    { getCustomerNames: getCustomerNames(orders) },
    { getOrderSummaries: getOrderSummaries(orders) },
    { getTotalRevenue: getTotalRevenue(orders) },
    { getCustomerSpending: getCustomerSpending(orders) },
    { sortByDate: sortByDate(orders) },
  ];
  setTimeout(() => {
...
const resultFunction = async () => {
  let data = await fetchAndProcess;
  console.table(data);
};
